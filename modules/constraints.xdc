...
## Matches fpga_top.v ports:
##   CLK100MHZ, CPU_RESETN, UART_RXD_OUT, UART_TXD_IN, led[15:0]
##
## Pin mappings taken from Digilent's official Nexys-A7-100T Master XDC:
...
create_generated_clock -name soc_clk -source [get_pins {clk_div2_reg/C}] -divide_by 2 [get_pins {u_clk_bufg/O}]
...
## Digilent net name: uart_txd_in
set_property -dict { PACKAGE_PIN C4 IOSTANDARD LVCMOS33 } [get_ports {UART_RXD_OUT}]
## FPGA TX to host / USB-UART RX
## Digilent net name: uart_rxd_out
set_property -dict { PACKAGE_PIN D4 IOSTANDARD LVCMOS33 } [get_ports {UART_TXD_IN}]

## These external paths are asynchronous / board-observable and do not
## participate in synchronous timing closure for the SoC core.
